...
static std::string MakeSnapFilePath(int ch)
{
    ...
    std::snprintf(
        fileName,
        sizeof(fileName),
        "ch%d_%04d%02d%02d_%02d%02d%02d.jpg",
        ch,
        t.tm_year + 1900,
        t.tm_mon + 1,
        t.tm_mday,
...
void* MultipthreadJpeg(void* argv)
{
    int ch = 0;
    if (argv != nullptr)
    {
        ch = static_cast<int>(reinterpret_cast<intptr_t>(argv));
    }
    int ret = SnapOneChannel(ch);
    std::printf("MultipthreadJpeg: ch=%d, ret=%d\n", ch, ret);
    return nullptr;
}
