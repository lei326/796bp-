...
{
	int iRet = 0;
	unsigned int iLocalIndex = 0;//cTmpPath 数组下标，用于拷贝字符串
	char cTmpPath[256];//用于逐级创建目录
	...
	if(_pcPath[iLocalIndex] == '/')
	{
		...
		cTmpPath[iLocalIndex] = _pcPath[iLocalIndex];//跳过根目录的首个斜杠(如果有)
		iLocalIndex++;
	}
	...
	while (iLocalIndex < sizeof(cTmpPath))
	{
		...
		if(cTmpPath[iLocalIndex] == '/')//遇到一级中间目录
		{
			cTmpPath[iLocalIndex] = '\0';//先把字符串截断以便调用函数创建目录

			iRet = stat(cTmpPath,&TDevState);
			if(iRet != 0)//如果目录不存在，尝试创建
			{
				iRet = mkdir(cTmpPath, _iMode);
				printf("[%s] creating directory %s, iRet = %d\n", __func__, cTmpPath, iRet);
			}

			cTmpPath[iLocalIndex] = '/';//再把字符串复原
		}
	...
	//返回值就以最后一次创建目录时的返回值为准
	//注意，确实存在创建中间目录失败而创建最终目录成功的情况
	//因为如果中间目录已经存在，就会返回失败，但这不是真失败
	return iRet;
}
...
if (frameCnt > (framerate * 10))
{
    ...
    // 关键：统一拼装 3 图 + 1 视频后，只回调一次
    pthread_mutex_lock(&g_attachLock);

    // 如果这一轮缓存还没准备好，也尽量按已有的拼
...
int AlarmMutimedia::Doordvr_Record_StartMp4(VI_CHN channel, DVR_U8_T encoder_size, AiAlarmItem *param)
{
    ...
    while (!quit)
    {
        if (VideoLost_GetEvent(channel) == false)
        {
            ...
            // 新一轮报警：重置缓存（会清 self->m_event_alarm 附件字段）
            if (g_cachedAlarmTime == 0 || g_cachedAlarmTime != param->alarmtime)
            {
                ResetAttachCache(this, param->alarmtime);
            }
            ...
            // ================================
            // 关键改动：只缓存 MP4 路径
            // 绝对不写 m_event_alarm.attachfile[]
            // 绝对不改 m_event_alarm.attachmentnum
            // ================================
            pthread_mutex_lock(&g_attachLock);
            ...
            for (i = framenum; i > 0; i--)
            {
                ...
                if (m_channelFramecount >= 13 * framerate)
                {
...
#endif // preframe block// 1

            // ================================
            // 注册 5 秒后数据回调
            // 真正的 3图+1视频组包回调在 AdasProcessVideoDataBehind5S 里做
            // ================================
